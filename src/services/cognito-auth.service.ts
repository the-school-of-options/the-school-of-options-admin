...
export const authService = {
  ...
  loginUser: async (
    ...
    try {
      return await authService.getTokens(AuthFlowType.USER_PASSWORD_AUTH, {
  ...
  getTokens: async (
    ...
    try {
      ...
      let response: { AuthenticationResult?: AuthenticationResultType; ChallengeName?: string };

      // Use initiateAuth for USER_PASSWORD_AUTH, adminInitiateAuth for ADMIN flows
      if (authFlow === AuthFlowType.USER_PASSWORD_AUTH) {
        response = await cognitoIdentityServiceProvider.initiateAuth({
          ClientId: clientId,
          AuthFlow: authFlow,
          AuthParameters: authParams,
        });
      } else {
        response = await cognitoIdentityServiceProvider.adminInitiateAuth({
          UserPoolId: userPoolId,
          ClientId: clientId,
          AuthFlow: authFlow,
          AuthParameters: authParams,
...
export const authenticateWithCognito = async (
  ...
  try {
    ...
    // Find the Cognito user to get their username
    const cognitoUser = await authService.findUserByEmail(email);
    if (!cognitoUser || !cognitoUser.Username) {
      return null;
    }

    // Find user in database by email
    const dbUser = await userRepository.findOne({ where: { email: email.toLowerCase() } });
    ...
    if (dbUser.role !== "Super-Admin") {
      console.log("User is not a Super-Admin:", email, "Role:", dbUser.role);
      return null;
    }

    // Get user's Cognito groups
    const userGroups = await authService.getUserRoleFromCognitoGroups(cognitoUser.Username);
