...
export const RegisterAdminJS = async () => {
  ...
  const Components = {
    ExportCsv: componentLoader.add('ExportCsv', path.join(__dirname, '../components/export-csv.component')),
  };
        ...
        options: {
          ...
          actions: {
            ...
            exportCsv: {
              ...
              icon: "Download",
              label: "Export to CSV",
              component: Components.ExportCsv, // Add the component reference
              handler: async (request: any, response: any, context: any) => {
                ...
                try {
                  ...
                  filters["filters.role"] = UserRole.SUPER_ADMIN; // Ensure we're only exporting super admins
                  
                  const { records } = await resource.find(filters, {
                    limit: 10000,
                  });
            ...
            exportCsv: {
              ...
              handler: async (request: any, response: any, context: any) => {
                ...
                try {
                  // Get all records based on current filters
                  const filters = request.query || {};
                  filters["filters.role"] = UserRole.USER; // Ensure we're only exporting regular users
